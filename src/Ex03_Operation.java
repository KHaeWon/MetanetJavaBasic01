...
/*
	...
	아 이거 opr을 opr.equals 이렇게 써야 되는 듯
...
public class Ex03_Operation {
	...
	public static void main(String[] args) {
		...
		if(opr.equals("+")) {
			System.out.println("연산결과 :"+(Integer.parseInt(num1) + Integer.parseInt(num2)));
		}else if(opr.equals("-")) {
			System.out.println("연산결과 :"+(Integer.parseInt(num1) - Integer.parseInt(num2)));
		}else if(opr.equals("*")) {
			System.out.println("연산결과 :"+(Integer.parseInt(num1) * Integer.parseInt(num2)));
		...
		//switch 문으로 하는 방법. 단, equals를 사용하지 않았으므로 이건 안 되겠지!!
//		switch (opr) {
//		case "+":
//			System.out.println("연산결과 :"+(Integer.parseInt(num1) + Integer.parseInt(num2)));
//			break;
...
//		}
		...
		/*
		 * byte + short -> 내부적으로 컴파일러가 int + int로 계산함
		 * char + char -> int + int
		 * =>즉, int보다 작은 정수 타입들은 다 int 형태로 바꾸어서 연산 처리
		 * 
		 * 정수 long은 예외
		 * char + int -> int
		 * int + int -> int
		...
		//(p==p2)이 참이면 p, 거짓이면 p2
		
		//javascript의 ===은 타입과 값이 같아야 true
